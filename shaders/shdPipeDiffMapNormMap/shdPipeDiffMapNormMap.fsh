...
void main()
{
    ...
    lightColour += vec3(0.8, 0.6, 0.4)*max(0.0, dot(n, normalize(vec3(1.0,  0.2,  0.6))));
    ...
    vec3 ng = normalize( v_vNormal );
    t = normalize( t - ng * dot(ng, t) );
    gl_FragColor.rgb = 0.5 + 0.5*v_vTangent;
    
    //gl_FragColor = vec4(vec3(max(0.0, dot(n, normalize(vec3(0.0,  0.0,  1.0))))), 1.0);
    
    //if (u_fNoNormalMap >= 0.5) gl_FragColor = texture2D(u_sNormal, v_vTexcoord);
