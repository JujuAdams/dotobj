	            ...
	            if (_value != 32)
	            {
	                ...
	                else switch(_line_data_list[| 0]) //Use the first piece of data we read to determine what kind of line this is
	                {
	                    ...
	                    case "bump": //"Bump" map (normal map)
	                        ...
	                        _texture_array = (_sprite >= 0)? new dotobj_class_texture(_sprite, 0, _line_data_list[| 1]) : undefined;
                        
	                        switch(_line_data_list[| 0])
	                        {
	                            case "map_Ka":    _material_struct.ambient_map      = _texture_array; break;
	                            case "map_Kd":    _material_struct.diffuse_map      = _texture_array; break;
	                            case "map_Ks":    _material_struct.specular_map     = _texture_array; break;
	                            case "map_Ke":    _material_struct.emissive_map     = _texture_array; break;
	                            case "map_Ns":    _material_struct.specular_exp_map = _texture_array; break;
	                            case "map_d":     _material_struct.dissolve_map     = _texture_array; break;
	                            case "map_decal":
	                            case "decal":     _material_struct.decal_map        = _texture_array; break;
	                            case "map_disp":                                
	                            case "disp":      _material_struct.ambient_map      = _texture_array; break;
	                            case "map_bump":                                
	                            case "bump":      _material_struct.normal_map       = _texture_array; break;
	                        }
	                    break;
